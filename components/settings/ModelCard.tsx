...
export const ModelCard: React.FC<ModelCardProps> = ({
  ...
  <div className={`p-3 rounded border flex flex-col gap-2 transition-colors ${isDarkMode ? 'bg-white/5 border-white/10 hover:bg-white/10' : 'bg-gray-50 border-gray-200 hover:bg-gray-100'}`}>
    <div className="flex items-center gap-2 mb-1">
      <div className={`p-1.5 rounded ${isDarkMode ? 'bg-teal-900/30 text-teal-400' : 'bg-teal-100 text-teal-700'}`}>
        <Icon size={16} />
      </div>
      <div>
        <h4 className={`text-xs font-bold uppercase tracking-wider ${isDarkMode ? 'text-teal-400' : 'text-teal-600'}`}>{title}</h4>
        <p className={`text-[9px] font-mono ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>{desc}</p>
      </div>
    </div>
    <div className="relative">
      ...
      <button
        onClick={onReset}
        className="absolute right-1 top-1/2 -translate-y-1/2 text-[9px] px-1 opacity-50 hover:opacity-100"
